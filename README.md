...
- 小米账号凭据、服务令牌、通知密钥、地图密钥等敏感配置，在写入数据库前会进行加密。页面及接口不会直接返回其明文内容。
- 正常首次部署时，应用会为该实例生成并维护一份独立的字段加密密钥环。不同部署实例之间不共享密钥，因此一个实例产生的密文不能直接迁移到另一个实例使用。
- 这里的“独立”是指每个部署实例拥有一份独立密钥环，并非为每个平台账号分别生成一把主密钥。密文还会与所属用户、配置字段和通知平台等上下文绑定，复制到其他用户、字段或实例后无法直接解密。
- 密钥环默认保存在部署配置目录的 `.secrets/field-keyring.json` 中。请将数据库与该配置目录一并妥善备份；密钥环丢失、被覆盖或与数据库不匹配后，已加密的敏感配置将无法恢复。
- 系统生成的完整备份包 `.xmbak` 都包含恢复所需的数据库和加密材料。请将备份文件与恢复口令分开妥善保存，切勿提交至代码仓库、公开分享或提供给无关人员。
